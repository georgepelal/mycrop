...
import { ArrowLeft, Loader2, Wind, AlertTriangle, Info, ShieldAlert, CloudRain } from "lucide-react";
import LocationSearch from "../components/LocationSearch";
import {
  AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, Legend
} from "recharts";

interface CropLodgingShearProps {
...
export default function CropLodgingShear({ onNavigate }: CropLodgingShearProps) {
  ...
  const fetchData = async (lat: number, lng: number, name: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Failed to compute crop wind shear risks");
      }
  ...
  const chartData = data?.dates.map((date, index) => ({
    ...
    "Lodging Risk (%)": data.lodgingIndices[index],
    "Wind Speed (km/h)": data.maxWindSpeed[index],
    "Rain (mm)": data.dailyRain[index]
  })) || [];
        ...
        <div className="flex items-center gap-4 border-b border-transparent pb-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Biomechanical wind drag and soil anchorage structural failure models.
            </p>
          </div>
        </div>
      ...
      {!data && !loading && !error && (
        <div className="h-96 flex flex-col items-center justify-center border border-gray-100 rounded-3xl bg-slate-50 shadow-sm text-center px-4">
          ...
          <p className="text-sm text-slate-500 max-w-md">
            Combine predictive rain/soil saturation with maximum wind shear profiles to predict stem breakage in tall stalk crops.
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="h-96 flex flex-col items-center justify-center border border-gray-100 rounded-3xl bg-white shadow-sm">
          ...
          <p className="text-sm font-medium text-gray-500">
            Calculating aerodynamic stem drag coefficients...
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">
                Peak Lodging Risk Array
              </div>
              <div className={`text-5xl font-black ${
                data.peakRisk > 75 ? 'text-rose-600' : data.peakRisk > 40 ? 'text-amber-500' : 'text-emerald-500'
              } flex items-center gap-2 py-2`}>
                {data.peakRisk}%
              </div>
              <div className="text-[10px] text-slate-500 mt-2 flex items-center gap-1">
                <AlertTriangle className={`w-3.5 h-3.5 ${
                  data.peakRisk > 75 ? 'text-rose-500' : data.peakRisk > 40 ? 'text-amber-500' : 'text-emerald-500'
                }`} />
                Highest Probability within 7 Days
              </div>
            </div>
            ...
            <div className="grid grid-rows-2 gap-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xl font-bold text-slate-800">
                    {Math.max(...data.maxWindSpeed)} km/h
                  </div>
                </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                    Soil Saturation Ratio (Rainfall)
                  </div>
                  <div className="text-xl font-bold text-slate-800">
                    {Math.max(...data.dailyRain)} mm
                  </div>
                </div>
          ...
          <div className="bg-white border text-gray-900 border-gray-200 rounded-3xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Wind className="w-5 h-5 text-indigo-500" />
              Aerodynamic Shear & Breaking Forecast
            </h3>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748B' }} dx={-10} domain={[0, 100]} />
                  ...
                  <Area yAxisId="left" type="monotone" dataKey="Lodging Risk (%)" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#colorRisk)" />
                  <Area yAxisId="right" type="monotone" dataKey="Wind Speed (km/h)" stroke="#6366f1" strokeWidth={2} fillOpacity={1} fill="url(#colorWind)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`border rounded-2xl p-5 flex gap-4 ${
              ...
              <ShieldAlert className={`w-6 h-6 shrink-0 mt-0.5 ${
                data.peakRisk > 75 ? 'text-rose-600' : data.peakRisk > 40 ? 'text-amber-600' : 'text-emerald-600'
              }`} />
