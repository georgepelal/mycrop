...
export default function GbifSpeciesSuggest() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/gbif-species-suggest".includes("health") || "/api/gbif-species-suggest".includes("ip-geolocation")) {
          response = await fetch("/api/gbif-species-suggest");
        } else {
          response = await fetch("/api/gbif-species-suggest", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ query: "Panthera" }),
          });
        }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.GbifSpeciesSuggest") || "GBIF Species Suggest"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/gbif-species-suggest</p>
        </div>
      </div>
