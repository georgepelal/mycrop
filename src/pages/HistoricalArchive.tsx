...
import { History, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
...
export default function HistoricalArchive() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/openmeteo-historical-archive".includes("health") || "/api/openmeteo-historical-archive".includes("ip-geolocation")) {
          response = await fetch("/api/openmeteo-historical-archive");
        } else {
          response = await fetch("/api/openmeteo-historical-archive", {
            method: "POST",
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.HistoricalArchive") || "Historical Climate Archive"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/openmeteo-historical-archive</p>
        </div>
      </div>
