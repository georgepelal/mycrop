...
import { CalendarDays, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useAuth } from "../contexts/AuthContext";

export default function LocalPublicHolidays() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/local-public-holidays".includes("health") || "/api/local-public-holidays".includes("ip-geolocation")) {
          response = await fetch("/api/local-public-holidays");
        } else {
          response = await fetch("/api/local-public-holidays", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ countryCode: "US", year: new Date().getFullYear() }),
          });
        }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.LocalPublicHolidays") || "Local Public Holidays"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/local-public-holidays</p>
        </div>
      </div>
