...
import { Wind, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import LocationMapPicker from "../components/LocationMapPicker";

export default function AirQualityAerosols() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/air-quality-aerosols".includes("health") || "/api/air-quality-aerosols".includes("ip-geolocation")) {
          response = await fetch("/api/air-quality-aerosols");
        } else {
          response = await fetch("/api/air-quality-aerosols", {
            method: "POST",
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.AirQualityAerosols") || "Air Quality & Aerosols"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/air-quality-aerosols</p>
        </div>
      </div>
