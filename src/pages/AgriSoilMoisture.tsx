...
export default function AgriSoilMoisture() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/openmeteo-agri-soil".includes("health") || "/api/openmeteo-agri-soil".includes("ip-geolocation")) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.AgriSoilMoisture") || "Agri-Soil Moisture"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/openmeteo-agri-soil</p>
        </div>
      </div>
