...
import { ArrowLeft, Loader2, Sun, AlertTriangle, Info, ShieldAlert, Zap } from "lucide-react";
import LocationSearch from "../components/LocationSearch";
import {
  AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, Legend
} from "recharts";

interface ParPpfdDataProps {
...
export default function ParPpfdData({ onNavigate }: ParPpfdDataProps) {
  const [data, setData] = useState<ParData | null>(null);
  ...
  const fetchData = async (lat: number, lng: number, name: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Failed to evaluate Photosynthetically Active Radiation loads");
      }
  ...
  const chartData = data?.dates.map((date, index) => ({
    ...
    "DLI (mol/m²/day)": data.dailyLightIntegral[index],
    "Peak PPFD (μmol/m²/s)": data.peakPpfd[index],
    "Shortwave (MJ/m²/day)": data.shortwaveRadiationMJ[index]
  })) || [];
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-4 border-b border-transparent pb-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Photosynthetically Active Radiation (400-700nm band) & Daily Light Integrals.
            </p>
          </div>
        </div>
      ...
      {!data && !loading && !error && (
        <div className="h-96 flex flex-col items-center justify-center border border-gray-100 rounded-3xl bg-slate-50 shadow-sm text-center px-4">
          <Sun className="w-12 h-12 text-slate-300 mb-4 animate-pulse" />
          <h3 className="text-lg font-bold text-slate-700 mb-2">
            Select a Location
          </h3>
          <p className="text-sm text-slate-500 max-w-md">
            Evaluate the Photosynthetic Photon Flux Density (PPFD) available to crop canopies across global geographies.
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="h-96 flex flex-col items-center justify-center border border-gray-100 rounded-3xl bg-white shadow-sm">
          ...
          <p className="text-sm font-medium text-gray-500">
            Parsing quantum sensor action curves and orbital arrays...
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
              ...
              <div className={`text-5xl font-black ${
                data.avgDli < 12.0 ? 'text-indigo-600' : data.avgDli > 30.0 ? 'text-rose-500' : 'text-amber-500'
              } flex items-center gap-2 py-2`}>
                {data.avgDli} <span className="text-xl font-bold text-slate-400">mol/m²/d</span>
              </div>
              <div className="text-[10px] text-slate-500 mt-2 flex items-center gap-1">
                <AlertTriangle className={`w-3.5 h-3.5 ${
                  data.avgDli < 12.0 ? 'text-indigo-500' : data.avgDli > 30.0 ? 'text-rose-500' : 'text-amber-500'
                }`} />
                Aggregated 7-Day Solar Dose
              </div>
            </div>
            ...
            <div className="grid grid-rows-2 gap-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xl font-bold text-slate-800">
                    {Math.max(...data.peakPpfd)} μmol/m²/s
                  </div>
                </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                    Max Shortwave Radiation
                  </div>
                  <div className="text-xl font-bold text-slate-800">
                    {Math.max(...data.shortwaveRadiationMJ)} MJ/m²/d
                  </div>
                </div>
          ...
          <div className="bg-white border text-gray-900 border-gray-200 rounded-3xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Sun className="w-5 h-5 text-amber-500" />
              Agronomic Solar Energy Curves (7-Day)
            </h3>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorDLI" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area yAxisId="left" type="monotone" dataKey="DLI (mol/m²/day)" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorDLI)" />
                  <Area yAxisId="right" type="monotone" dataKey="Peak PPFD (μmol/m²/s)" stroke="#8b5cf6" strokeWidth={2} fillOpacity={1} fill="url(#colorPPFD)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`border rounded-2xl p-5 flex gap-4 ${
              ...
              <ShieldAlert className={`w-6 h-6 shrink-0 mt-0.5 ${
                data.avgDli < 12.0 ? 'text-indigo-600' : data.avgDli > 30.0 ? 'text-rose-600' : 'text-emerald-600'
              }`} />
              <div className="text-sm leading-relaxed">
                <strong className="block mb-1">Phenological Advisory</strong> 
                {data.advisory}
              </div>
            </div>
