...
import { Flag, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useAuth } from "../contexts/AuthContext";

export default function RegionalCountrySovereign() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/regional-country-sovereign".includes("health") || "/api/regional-country-sovereign".includes("ip-geolocation")) {
          response = await fetch("/api/regional-country-sovereign");
        } else {
          response = await fetch("/api/regional-country-sovereign", {
            method: "POST",
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 rounded-2xl bg-brand-green/10 flex items-center justify-center">
          <Flag className="w-6 h-6 text-brand-green" />
        </div>
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.RegionalCountrySovereign") || "Regional Country Data"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/regional-country-sovereign</p>
        </div>
      </div>
