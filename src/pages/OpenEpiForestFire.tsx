...
import { Flame, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import LocationMapPicker from "../components/LocationMapPicker";

export default function OpenEpiForestFire() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/openepi-forest-fire".includes("health") || "/api/openepi-forest-fire".includes("ip-geolocation")) {
          response = await fetch("/api/openepi-forest-fire");
        } else {
          response = await fetch("/api/openepi-forest-fire", {
            method: "POST",
