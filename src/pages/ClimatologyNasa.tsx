...
import { Sun, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useAuth } from "../contexts/AuthContext";
import LocationMapPicker from "../components/LocationMapPicker";

export default function ClimatologyNasa() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/climatology-nasa".includes("health") || "/api/climatology-nasa".includes("ip-geolocation")) {
          response = await fetch("/api/climatology-nasa");
        } else {
          response = await fetch("/api/climatology-nasa", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ lat: coords.lat, lng: coords.lng, startDaysAgo: 30 }),
          });
        }
