...
import { Bug, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useAuth } from "../contexts/AuthContext";
import LocationMapPicker from "../components/LocationMapPicker";

export default function GbifLocalOccurrences() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/gbif-local-occurrences".includes("health") || "/api/gbif-local-occurrences".includes("ip-geolocation")) {
          response = await fetch("/api/gbif-local-occurrences");
        } else {
          response = await fetch("/api/gbif-local-occurrences", {
            method: "POST",
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.GbifLocalOccurrences") || "GBIF Local Occurrences"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/gbif-local-occurrences</p>
        </div>
      </div>
