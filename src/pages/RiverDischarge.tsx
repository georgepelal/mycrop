...
import { Droplets, AlertCircle, MapPin as MapPinIcon, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import LocationMapPicker from "../components/LocationMapPicker";

export default function RiverDischarge() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        let response;
        if ("/api/openmeteo-river-discharge".includes("health") || "/api/openmeteo-river-discharge".includes("ip-geolocation")) {
          response = await fetch("/api/openmeteo-river-discharge");
        } else {
          response = await fetch("/api/openmeteo-river-discharge", {
            method: "POST",
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">{t("sidebar.RiverDischarge") || "River Discharge"}</h1>
          <p className="text-sm font-medium text-slate-500">Live API Endpoint: /api/openmeteo-river-discharge</p>
        </div>
      </div>
